...
/**
 * 整个大致流程
 * 1,nginx读取到配置文件时，发现upstreamtest模块
 * 2,调用ngx_http_upstreamtest_commands指定的ngx_http_upstreamtest回调函数
 * 3,ngx_http_upstreamtest回调时设置处理HTTP的回调函数ngx_http_upstreamtest_handler
 *
 */
static ngx_int_t ngx_http_upstreamtest_handler(ngx_http_request_t *r);
static char *
ngx_http_upstreamtest(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
...
static ngx_command_t ngx_http_upstreamtest_commands[] = {
        {
                ngx_string("upstreamtest"),
                NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_HTTP_LMT_CONF|NGX_CONF_NOARGS,
                //set回调函数，
                //char               *(*set)(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
                //当某个配置快中出现upstreamtest时，就会回调此函数
                ngx_http_upstreamtest,
